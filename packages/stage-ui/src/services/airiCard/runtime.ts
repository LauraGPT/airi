...
function compileLorebookEntries(
  ...
  const remaining = book.entries.map((entry, sourceIndex) => ({ entry, sourceIndex }))
  ...
  while (remaining.length > 0 && activatedInPass) {
    ...
    for (let index = remaining.length - 1; index >= 0; index -= 1) {
      ...
      const parsed = parseLorebookContent(candidate.entry.content)
...
function matchesAnyKey(keys: string[], text: string, useRegex: boolean, caseSensitive = false): boolean {
  ...
  if (useRegex) {
    ...
    for (const key of keys) {
      try {
        patterns.push(new RegExp(key, flags))
      }
      catch {
...
function replaceStableSystemPrompt(
  ...
  systemMessage.content = expandCharacterCardMacros(
    [content, ...additionalLore].filter(isNonEmptyString).join('\n\n'),
    card,
    options,
  )
}
